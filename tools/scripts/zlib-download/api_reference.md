...
Config set options:
- `--zlib-email` — Z-Library email (password must be set in `.config/book-tools/.env`)
- `--annas-key` — Anna's Archive API key (prefer setting `ANNAS_SECRET_KEY` in `.config/book-tools/.env`)
