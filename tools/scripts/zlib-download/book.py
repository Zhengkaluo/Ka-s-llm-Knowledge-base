...
Config stored at <knowledge-base-root>/.config/book-tools/
"""
...
# Config lives inside the knowledge base: .config/book-tools/
# SCRIPT_DIR is tools/scripts/zlib-download/, so go up 3 levels to reach KB root
KB_ROOT = SCRIPT_DIR.parent.parent.parent
CONFIG_DIR = KB_ROOT / ".config" / "book-tools"
...
def load_config() -> dict:
    ...
    # Proxy support (needed in China mainland)
    if env.get("HTTPS_PROXY"):
        cfg["proxy"] = env["HTTPS_PROXY"]
...
def _get_zlib():
    ...
    if remix_userid and remix_userkey:
        z = Zlibrary(remix_userid=remix_userid, remix_userkey=remix_userkey, proxies=proxies)
        if z.isLoggedIn():
            return z
    ...
    if email and password:
        z = Zlibrary(email=email, password=password, proxies=proxies)
        if z.isLoggedIn():
    ...
    if not email and not password and not remix_userid:
        die("Z-Library not configured. Edit .config/book-tools/.env with your ZLIB_EMAIL and ZLIB_PASSWORD.")
    else:
        die("Z-Library login failed. Check credentials in .config/book-tools/.env. If in China, set HTTPS_PROXY in .env.")
        ...
        if zlib_has_tokens:
            try:
                ...
                if z.isLoggedIn():
                    ...
                    zlib_status = "expired"
                    zlib_hint = "Cached tokens expired and no email/password available. Edit .config/book-tools/.env"
            except Exception:
                if zlib_has_email:
                    zlib_status = "configured"
